...
function displayNames(name, scientific_name) {
    let newArray = [];
    name, scientific_name.forEach();
}
...
function lowCaseAnimalNames(animal_name) {
    var lowercaseAnimalArr = [];
    animal_name.map(function(animal) {
        var animalName = animal.animal_name.toLowerCase();
        lowercaseAnimalArr.push(animalName);
    });
    return lowercaseAnimalArr;
}
...
const lowPopulationAnimals = () => {
    const result = lowPopulationAnimals.filter(
        lowPopulationAnimal => lowPopulationAnimal.length < 5
    );
};
...
*/
function populationTotal(callback) {
